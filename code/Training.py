...
class trainer():
    ...
    def start(self, episodes, stock_type, logging, env_debug, rl_debug, brain, ID, model_based):
        ...
        # Initiate new evironment and RL agent
            self.bike_station = environment(self.stock_type, debug = self.env_debug, ID = self.ID,
                                    station_history = self.station_history)
            self.sim_stock.append(self.bike_station.get_sim_stock())

            if self.brain == 'q':
                self.operator = agent(epsilon = 0.9, lr = 0.9, gamma = 0.9, 
                                  current_stock = self.bike_station.current_stock(), 
                                  debug = self.rl_debug,
                                  expected_stock = self.bike_station.get_expected_stock(),
                                  model_based = model_based)
            elif self.brain == 'dqn':
                self.operator = DeepQNetwork(self.bike_station.n_actions, self.bike_station.n_features, 0.9, 0.9)
            else:
    ...
    def cal_performance(self):
        ...
        for session in range(len(self.final_stocks)):
            ...
            num_overstock = np.count_nonzero(np.array(self.final_stocks[session]) > 70)
            num_understock = np.count_nonzero(np.array(self.final_stocks[session]) <= 30)
    ...
    def save_session_results(self, timestamp):
        ...
        fig1 = plt.figure()
        episodes_ =[self.episodes[0]]
        successful_stocking_=[successful_stocking[0]]
        for i in range(len(self.episodes)):
            if (i+1) % len(self.episodes)//5 == 0:
                episodes_.append(self.episodes[i])
                successful_stocking_.append(successful_stocking[i])
        plt.plot(episodes_, successful_stocking_)
        plt.xlabel("Episodes")
        ...
        for session in range(len(self.session_stock_history)):
            ...
            plt.axhline(y = 80, c = "r", ls = "--", label = "Upper Stock Limit")
            plt.axhline(y = 20, c = "r", ls = "--", label = "Lower Stock Limit")
